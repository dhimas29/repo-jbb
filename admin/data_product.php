...
if (isset($_GET['delete'])) {
  ...
  if (deleteProduct($id_produk)) {
    ...
    echo "<script>alert('gagal hapus');window.location.href='data_product.php'</script>";
  }
}
                          ...
                          while ($row = mysqli_fetch_assoc($result)) { ?>
                            <tbody>
                              <tr>
                                ...
                                <td><?= $row['margin'] ?></td>
                                <td><?= substr($row['des'], 0, 40) . "..." ?></td>
                                <td><img src="<?= ('../img/product/') . $row['gambar']; ?>" width='50' height='50'></td>
                                <td>
                                  <center>
                                    <a class="btn btn-primary" href="edit_product.php?id_product=<?= $row['id_produk'] ?>"></i>Ubah</a>
                                    <a class="btn btn-dafault" href="data_product.php?delete=true&id_product=<?= $row['id_produk'] ?>" onclick="return confirm ('yakin ingin hapus data?');"><i class="fa fa-trash-o"></i>Hapus</a>
                                  </center>
                                </td </tr> <?php $i++;
                                          } ?>
                            </tbody>
